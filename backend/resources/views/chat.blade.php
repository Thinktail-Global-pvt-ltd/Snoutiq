...
@php
  ...
  // ✅ Patient identity: use user_id from SESSION (fallback to auth()->id or 101)
...
<script>
  ...
  // ✅ Use SESSION user_id everywhere (Blade injected)
  const PATIENT_ID = Number(@json($patientId));
  console.log('PATIENT_ID (from PHP session user_id/auth):', PATIENT_ID);
</script>
