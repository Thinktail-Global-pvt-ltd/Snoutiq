...
const Dashboard = () => {
  ...
  // Scroll to bottom
  const scrollToBottom = useCallback((behavior = "smooth") => {
    if (!messagesEndRef.current?.parentNode) return;
    const parent = messagesEndRef.current.parentNode;
    parent.scrollTo({ top: parent.scrollHeight, behavior });
  }, []);

  // Fetch chat history
  const fetchChatHistory = useCallback(async (roomToken) => {
    if (!user?.id) return;
    setIsLoading(true);
    try {
      const token = localStorage.getItem("token");
  ...
  // Load chat history on mount or room change
  useEffect(() => {
    if (updateChatRoomToken && chat_room_token) updateChatRoomToken(chat_room_token);
    fetchChatHistory(currentChatRoomToken);
  }, [currentChatRoomToken, chat_room_token, fetchChatHistory, updateChatRoomToken]);

  // Typing animation
  const startTypingAnimation = useCallback((messageId, fullText) => {
    if (typingTimeouts.current.has(messageId)) clearTimeout(typingTimeouts.current.get(messageId));
    let charIndex = 0;
    const typeNext = () => {
      if (charIndex >= fullText.length) return;
      const nextIndex = Math.min(charIndex + 2, fullText.length);
      setMessages(prev => prev.map(m => m.id === messageId ? { ...m, displayedText: fullText.slice(0, nextIndex) } : m));
      charIndex = nextIndex;
      const timeoutId = setTimeout(typeNext, 15);
      typingTimeouts.current.set(messageId, timeoutId);
    };
    typeNext();
  }, []);
  ...
  const handleSendMessage = useCallback(async (inputMessage) => {
    ...
    const loaderId = "__loader__";
    const userMsg = { id: genId(), text: inputMessage, sender: "user", timestamp: new Date() };
    const loaderMsg = { id: loaderId, type: "loading", sender: "ai", text: "", timestamp: new Date() };
    ...
    try {
      ...
      const payload = {
        ...
        pet_breed: "Unknown",
        pet_age: user?.pet_age?.toString() || "Unknown",
        pet_location: "Unknown",
      };

      const res = await axios.post("https://snoutiq.com/backend/api/chat/send", payload, { headers: { Authorization: `Bearer ${token}` }, timeout: 30000 });
      const answer = res.data?.chat?.answer || "";
      if (res.data?.context_token) setContextToken(res.data.context_token);

      setMessages(prev => prev.map(m => m.id === loaderId ? { id: genId(), sender: "ai", text: answer, displayedText: "", timestamp: new Date() } : m));
      startTypingAnimation(genId(), answer);
    } catch (err) {
      console.error(err);
  ...
  }, [sending, currentChatRoomToken, contextToken, startTypingAnimation, user?.id, user?.pet_name, user?.pet_age]);
    ...
    <div className="flex flex-col h-full">
      <div className="bg-white border-b border-gray-200 px-6 py-4 shadow-sm flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-1 text-sm">{currentChatRoomToken ? `Ask questions about ${user?.pet_name || "your pet"}'s health` : ""}</p>
        </div>
        {messages.length > 0 && <button onClick={clearChat} className="text-sm bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-1 rounded-md transition-colors" disabled={sending}>Clear Chat</button>}
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 bg-gray-50">
        <div className="max-w-4xl mx-auto py-4">
          {isLoading ? (
            ...
            <div className="space-y-4">
              {messages.map((msg, idx) => <MessageBubble key={msg.id || idx} msg={msg} index={idx} />)}
              <div ref={messagesEndRef} />
            </div>
          )}
        </div>
  ...
  ), [messages, sending, isLoading, handleSendMessage, clearChat, currentChatRoomToken, user?.pet_name]);
  ...
  return (
    <>
      ...
      <div className="hidden lg:flex h-[calc(100vh-64px)] mt-16 bg-gray-50">
        <div className="w-64 bg-white border-r border-gray-200 shadow-sm overflow-y-auto">
          <Sidebar onNewChat={() => fetchChatHistory()} />
        </div>
