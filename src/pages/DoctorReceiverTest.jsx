...
export default function DoctorReceiverTest() {
  ...
  const cleanup = () => {
    if (channelRef.current) {
      channelRef.current.stopListening('.CallRequested');
  ...
  const connectAndListen = () => {
    ...
    const chan = echo.channel(`doctor.${doctorId}`);
    channelRef.current = chan;
    chan
      .listen('.CallRequested', (e) => {
        log('CallRequested', JSON.stringify(e));
        if (e?.call_id) setCallId(e.call_id);
        if (e?.patient_id) setPatientId(e.patient_id);
        setCallStatus(e?.status || 'ringing');
      })
  ...
  const postAction = async (action) => {
    if (!callId) {
      log(action, 'no call_id');
      return;
    }
    try {
      const data = await apiPost(`/api/calls/${callId}/${action}`);
      log(`${action} ok`, JSON.stringify(data));
    } catch (error) {
  ...
  return (
    <div style={{ padding: 20, maxWidth: 900, margin: '0 auto' }}>
      <h1 style={{ fontSize: 26, marginBottom: 8 }}>Doctor Receiver Test</h1>
      <p style={{ color: '#4b5563', marginBottom: 16 }}>
        Backend: {apiBaseUrl()} — Subscribe to <code>doctor.{'{id}'}</code> for <code>.CallRequested</code> and{' '}
        <code>.CallStatusUpdated</code>.
      </p>

      <div style={cardStyle}>
        <div style={{ display: 'grid', gap: 12, gridTemplateColumns: '1fr 1fr 1fr' }}>
          <div>
            <label style={{ fontWeight: 700, marginBottom: 4, display: 'block' }}>Doctor ID</label>
            <input
              style={inputStyle}
              type="number"
              value={doctorId}
          ...
          <div>
            ...
            <input style={inputStyle} value={patientId} onChange={(e) => setPatientId(e.target.value)} />
          </div>
        </div>
        <div style={{ display: 'grid', gap: 10, gridTemplateColumns: 'repeat(3,1fr)', marginTop: 12 }}>
          ...
          <button style={buttonStyle(false)} onClick={() => postAction('accept')}>
            Accept
          </button>
          <button style={buttonStyle(false)} onClick={() => postAction('reject')}>
            Reject
          </button>
          <button style={buttonStyle(false)} onClick={() => postAction('end')}>
            End
          </button>
        </div>
        <div style={{ marginTop: 12, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          <span style={{ padding: '6px 10px', borderRadius: 8, background: subscribed ? '#dcfce7' : '#fee2e2' }}>
            Channel: {subscribed ? `doctor.${doctorId}` : 'not subscribed'}
          </span>
          <span style={{ padding: '6px 10px', borderRadius: 8, background: '#e0f2fe' }}>
            Status: {callStatus}
          </span>
      ...
      <div style={cardStyle}>
        <h3 style={{ marginTop: 0 }}>Incoming Call</h3>
        <p style={{ fontWeight: 700, fontSize: 18, margin: '4px 0' }}>
          {callId ? `Call ${callId}` : 'Waiting...'}
        </p>
        <p style={{ color: '#4b5563' }}>
          Patient: {patientId || '—'} — Status: <strong>{callStatus}</strong>
        </p>
      </div>
