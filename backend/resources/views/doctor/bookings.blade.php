...
@section('content')
  @php $debug = request()->query('debug') === '1'; @endphp
...
<script>
  ...
  async function loadDoctorDropdown(){
    try{
      const res = await api(`${apiBase}/doctors`);
      console.log('[doctor-bookings] GET /doctors =>', res);
      if(!res.ok || !Array.isArray(res.json?.doctors)) return;
      const options = res.json.doctors
  ...
  document.addEventListener('DOMContentLoaded', ()=>{
    ...
    if(doctorSelect){ doctorSelect.addEventListener('change', ()=>{ if(doctorInput) doctorInput.value = doctorSelect.value; load(); }); }
    if(doctorInput && doctorSelect){ doctorInput.addEventListener('input', ()=>{ const val = doctorInput.value; const opt = Array.from(doctorSelect.options).find(o => o.value === val); doctorSelect.value = opt ? opt.value : ''; }); }
