...
<div class="max-w-5xl mx-auto">
  ...
  {{-- =================== Find Slots Near Me (keep only PINCODE) =================== --}}
  <div class="mt-6">
    <div class="bg-white rounded-xl shadow p-4">
      <div class="flex items-center justify-between mb-3">
        <h3 class="text-base font-semibold text-gray-800">Find Slots Near Me (Session Location)</h3>
        <div class="flex items-center gap-2">
          <input type="date" id="near_date" class="rounded border-gray-300 text-sm" value="{{ date('Y-m-d') }}">
          {{-- ✅ Only this button is kept --}}
      ...
      <div class="mt-5 border-t pt-3">
        <div class="flex items-center justify-between">
          <h4 class="text-sm font-medium text-gray-800">Doctor Schedule (Booked Slots)</h4>
          <span class="text-[11px] text-gray-500">For selected date (IST)</span>
        </div>
        <div id="bookedMeta" class="text-xs text-gray-600 mt-1"></div>
        <div id="bookedSlots" class="mt-2 text-sm"></div>
      </div>
    </div>
  </div>
    ...
    (function(){
      ...
      function renderSlotsPin(list, strip){
        ...
        list.forEach(s=>{
          ...
          html += card(`
            <div class="text-xs text-gray-600">Hour (IST): <b>${hh}:00</b> - ${stripTxt} - Role: ${s.role}</div>
      ...
      function renderBooked(list){
        if (!list || !list.length) return '<div class="text-xs text-gray-500">No booked slots.</div>';
        let html = '<div class="grid sm:grid-cols-2 gap-2">';
        list.forEach(s=>{
          const hh = (s.ist_hour ?? ((Number(s.hour_24)+6)%24));
          const role = s.role || '-';
          const status = String(s.status||'').replace(/_/g,' ');
          html += card(`<div class="text-xs text-gray-600">Hour (IST): <b>${String(hh).padStart(2,'0')}:00</b> - Strip: #${s.strip_id} - Role: ${role} - Status: <b>${status}</b></div>`);
        });
        html += '</div>';
        return html;
      }
      ...
      async function loadBooked(){
        const date = el('#near_date')?.value;
        const doctorId = Number(document.querySelector('#doctor_id')?.value || 0);
        if(!date || !doctorId){
          el('#bookedMeta').textContent = 'Select date and doctor.';
          el('#bookedSlots').innerHTML = '';
          return;
        }
        const url = `${apiBase}/video/slots/doctor?doctor_id=${doctorId}&date=${encodeURIComponent(date)}&tz=IST`;
        const r = await fetch(url, { credentials:'include' });
        const j = await r.json().catch(()=>({}));
        const list = Array.isArray(j?.slots) ? j.slots : [];
        el('#bookedSlots').innerHTML = renderBooked(list);
        el('#bookedMeta').innerHTML = `Doctor ID: <b>${doctorId}</b> • Booked: <b>${list.length}</b> slot(s)`;
      }
      ...
      window._commitNearSlot = async function(slotId){
        try{
          ...
          if (r.ok){
            ...
            // only PINCODE flow kept, so re-run that + booked
            findNearByPincode();
      ...
      document.addEventListener('DOMContentLoaded', ()=>{
        ...
        // ✅ Only the PINCODE button is wired
        document.querySelector('#btnNearFindPin')?.addEventListener('click', ()=>{ findNearByPincode(); loadBooked(); });
        document.querySelector('#doctor_id')?.addEventListener('change', loadBooked);
        document.querySelector('#near_date')?.addEventListener('change', loadBooked);
        // initial load of booked once
        loadBooked();
      });
    })();
  </script>
  ...
  <script>
    (function(){
      ...
      async function loadSlots(){
        ...
        try{
          const r = await fetch(`${apiBase}/video-schedule/doctors/${id}/free-slots?date=${encodeURIComponent(d)}`, { credentials:'include' });
      ...
      document.addEventListener('DOMContentLoaded', ()=>{
        const dd = document.querySelector('#doctor_id'); if(dd && dd.options.length && dd.value) loadExisting();
  ...
  <div class="max-w-5xl mx-auto mt-8">
    <div class="bg-white rounded-xl shadow p-4">
      <div class="flex items-center justify-between mb-2">
        ...
        <div class="flex items-center gap-2">
          <input type="date" id="pin_date" class="rounded border-gray-300 text-sm">
          <button id="btnPinLoad" class="px-3 py-1.5 rounded bg-gray-800 text-white text-sm">Load Pincodes</button>
        </div>
      </div>
    ...
    <script>
      (function(){
        ...
        function renderPinMatrix(matrix, pinRows){
          ...
          pinRows.forEach(r=>{
            html += `<tr class="border-t"><td class="px-2 py-1 whitespace-nowrap font-medium">${r.code} - ${r.label||''}</td>`;
            hoursIST.forEach(h=>{
          ...
          if (sum){
            ...
            sum.innerHTML = `Committed: <b>${totals.committed||0}</b> | In-progress: <b>${totals.in_progress||0}</b> | Done: <b>${totals.done||0}</b> <span class="text-gray-500">(Last updated ${stamp})</span>`;
          }
        }
    ...
    <div class="bg-white rounded-xl shadow p-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
        <div>
          ...
          <input type="date" id="optin_date" class="mt-1 w-full rounded border-gray-300">
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700">Max slots to claim</label>
          <input type="number" id="optin_max" min="1" max="12" value="2" class="mt-1 w-full rounded border-gray-300">
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700">Preference</label>
          <select id="optin_pref" class="mt-1 w-full rounded border-gray-300">
        ...
        <div class="flex items-end">
          <button id="btnOptin" class="w-full md:w-auto px-4 py-2 rounded bg-teal-600 text-white hover:bg-teal-700">
            Auto-claim Tonight
          </button>
        </div>
      </div>
        ...
        async function autoClaimTonight(){
          ...
          try{
            ...
            for (const s of targets){
              ...
              else { failCount++; logOpt(`⚠️ failed (#${s.id}): ${res.reason}`); }
