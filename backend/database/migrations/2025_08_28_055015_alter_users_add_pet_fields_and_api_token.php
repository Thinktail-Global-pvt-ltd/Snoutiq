...
return new class extends Migration {
    public function up(): void
    {
        Schema::table('users', function (Blueprint $table) {
            // safe-guards so reruns don't fail
            ...
            // pet fields
            if (!Schema::hasColumn('users', 'pet_name'))   $table->string('pet_name', 120)->nullable()->after('role');
            if (!Schema::hasColumn('users', 'pet_gender')) $table->string('pet_gender', 20)->nullable()->after('pet_name'); // male|female|others|unknown
            if (!Schema::hasColumn('users', 'pet_age'))    $table->unsignedTinyInteger('pet_age')->nullable()->after('pet_gender'); // years

            if (!Schema::hasColumn('users', 'pet_doc1'))   $table->string('pet_doc1')->nullable()->after('pet_age'); // file path or URL
            if (!Schema::hasColumn('users', 'pet_doc2'))   $table->string('pet_doc2')->nullable()->after('pet_doc1');

            // random bearer token (hash stored)
    ...
    {
        Schema::table('users', function (Blueprint $table) {
            // drop in reverse (optional — keep if needed)
