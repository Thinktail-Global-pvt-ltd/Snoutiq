...
export default function SymptomCheckerSidebar({ isOpen, setIsOpen, activeChatRoomToken, onSelectChat, onNewChat, historyRefreshKey }) {
  ...
  return (
    <div 
      className={`fixed inset-y-0 left-0 z-40 flex w-64 shrink-0 flex-col border-r border-slate-200 bg-white transition-transform duration-300 md:relative md:translate-x-0 ${
    ...
    >
      ...
      <div className="p-3">
        ...
        <button 
          onClick={onNewChat}
          className="flex w-full items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-50 shadow-sm transition-all"
        >
          <Plus size={16} />
          New chat
        </button>
      ...
      <div className="border-t border-slate-100 p-4 mt-auto">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 text-sm font-medium text-slate-700">
            ...
            <span className="truncate">{userId ? (authState?.user?.name || "User") : "Guest User"}</span>
          </div>
          {userId && (
            <button 
