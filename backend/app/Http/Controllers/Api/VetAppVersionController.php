...
class VetAppVersionController extends Controller
{
    private const APP_KEY = 'vet-app';
    private const DEFAULT_TITLE = 'Update required';
    private const DEFAULT_MESSAGE = 'Please update your app to continue using SnoutIQ Vet.';

    public function show(Request $request)
    {
        $versions = VetAppVersion::query()
            ->where('app_key', self::APP_KEY)
    ...
    public function adminList()
    {
        ...
        return response()->json([
            'success' => true,
            'data' => $versions->map(fn (VetAppVersion $version) => $this->adminPayload($version))->values(),
        ]);
    }

    public function update(Request $request, int $id)
    {
        $version = VetAppVersion::query()->find($id);

        if (! $version) {
            return response()->json([
                'success' => false,
                'message' => 'Vet app version not found.',
            ], 404);
        }
        ...
        return response()->json([
            'success' => true,
            'message' => 'Vet app version updated successfully.',
            'data' => $this->adminPayload($version->refresh()),
        ]);
    }

    private function publicPlatformPayload(VetAppVersion $version): array
    {
        return [
    ...
    private function adminPayload(VetAppVersion $version): array
    {
        return [
            'id' => $version->id,
