      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="flex items-start gap-2 pt-1">
          ...
          <label for="create-price-after" class="text-sm font-semibold text-gray-700">
            Price after service
            <span class="block text-xs font-normal text-gray-500">Hide price and collect it after completing the service.</span>
          </label>
        </div>
        @unless($isOnboarding)
          <div>
...
<script>
  ...
  const openCreate = () => { resetCreateForm(); open(createModal); applyCreatePriceState(); };
  ...
  function initPriceToggle(form){
    ...
    const update = () => {
      ...
      if (priceWrapper) priceWrapper.classList.toggle('hidden', after);
      if (priceInput) {
