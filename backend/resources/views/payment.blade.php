...
@php
  ...
  $amountQ   = request()->query('amount'); // optional; fallback 499 in JS
@endphp
...
<script>
  ...
  function loadRazorpay() {
    return new Promise(res => {
      if (window.Razorpay) { razorpayLoaded = true; res(true); return; }
      const script = document.createElement('script');
      script.src = 'https://checkout.razorpay.com/v1/checkout.js';
      script.onload = () => { razorpayLoaded = true; res(true); };
      script.onerror = () => { razorpayLoaded = false; res(false); };
  ...
  function renderTimeout() {
    app.innerHTML = `
      <div class="bg-red-50 rounded-2xl shadow-xl p-8 max-w-md w-full text-center">
        <svg class="w-16 h-16 text-red-500 mx-auto mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01M12 5a7 7 0 100 14 7 7 0 000-14z"/></svg>
        ...
        <a href="/chat" class="inline-block w-full py-3 bg-red-600 text-white rounded-lg hover:bg-red-700">Back to Chat</a>
      </div>`;
  }

  function renderDoctorCancelled() {
    app.innerHTML = `
      <div class="bg-yellow-50 rounded-2xl shadow-xl p-8 max-w-md w-full text-center">
        <svg class="w-16 h-16 text-yellow-500 mx-auto mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01M12 5a7 7 0 100 14 7 7 0 000-14z"/></svg>
        <h2 class="text-2xl font-bold text-yellow-800 mb-4">Call Cancelled</h2>
        <p class="text-yellow-700 mb-6">The doctor has cancelled the call. No payment has been charged.</p>
        <a href="/chat" class="inline-block w-full py-3 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700">Back to Chat</a>
      </div>`;
  }
        ...
        <div class="bg-gradient-to-r from-indigo-600 to-purple-600 p-6 text-white">
          <div class="flex items-center justify-between">
            <div class="flex items-center">
              <svg class="w-8 h-8 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
              <div>
          ...
          <div class="space-y-6">
            <div class="bg-green-50 border border-green-200 rounded-xl p-5">
              <div class="flex items-center mb-2">
                <svg class="w-6 h-6 text-green-600 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4"/></svg>
          ...
          <div class="space-y-6">
            <div class="bg-white rounded-xl p-5 border border-gray-200">
              ...
              <div class="grid grid-cols-2 gap-3 mb-6">
                ...
                <div class="p-4 border rounded-lg border-gray-300"><div class="flex items-center"><span class="text-2xl mr-2">📱</span><div><p class="font-medium text-gray-900 text-sm">UPI</p><p class="text-xs text-gray-500">Pay using UPI apps</p></div></div></div>
                <div class="p-4 border rounded-lg border-gray-300"><div class="flex items-center"><span class="text-2xl mr-2">🏦</span><div><p class="font-medium text-gray-900 text-sm">Net Banking</p><p class="text-xs text-gray-500">Pay using net banking</p></div></div></div>
                <div class="p-4 border rounded-lg border-gray-300"><div class="flex items-center"><span class="text-2xl mr-2">💰</span><div><p class="font-medium text-gray-900 text-sm">Wallet</p><p class="text-xs text-gray-500">Pay using wallet</p></div></div></div>
              </div>

              <div class="flex items-center justify-center p-3 bg-gray-50 rounded-lg border border-gray-200">
                <svg class="w-5 h-5 text-green-600 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 11c0 1.657-1.343 3-3 3S6 12.657 6 11s1.343-3 3-3 3 1.343 3 3z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7v10a2 2 0 01-2 2H7a2 2 0 01-2-2V7"/></svg>
                <span class="text-sm text-gray-600">Secure & encrypted payment processing</span>
              </div>
            </div>

            <button id="pay-btn" class="w-full py-4 px-6 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-xl hover:from-indigo-700 hover:to-purple-700 transition-all duration-200 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center">
              <svg class="w-5 h-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h10M5 6h14"/></svg>
  ...
  function startTimer() {
    const timerEl = $('#timer');
    const expire = $('#expire-note');
    const t = setInterval(() => {
      timeLeft--;
      if (timerEl) timerEl.textContent = formatTime(timeLeft);
      if (timeLeft === 60 && expire) {
        expire.textContent = `Hurry! Payment window expires in ${formatTime(timeLeft)}`;
        expire.classList.remove('hidden');
      }
      if (timeLeft <= 0) {
        clearInterval(t);
        paymentStatus = 'timeout';
        socket.emit('payment-cancelled', { callId: CALL_ID, patientId: PATIENT_ID, doctorId: DOCTOR_ID, reason: 'timeout' });
        renderTimeout();
      }
    }, 1000);
  }
  ...
  async function handlePayment(){
    if (!window.Razorpay) { showStatus('bg-red-50 border border-red-200 text-red-800', 'Gateway failed to load. Please refresh.'); return; }
    if (loading) return;
    loading = true;
    ...
    try {
      // 1) Create Order
      const res = await fetch('https://snoutiq.com/backend/api/create-order', {
        method: 'POST',
        headers: { 'Content-Type':'application/json' },
        body: JSON.stringify({
          amount: doctorInfo.amount,       // rupees (backend may convert to paise)
          callId: CALL_ID,
          doctorId: DOCTOR_ID,
      ...
      const options = {
        ...
        handler: async (rp) => {
          try {
            // 3) Verify payment
            const verify = await fetch('https://snoutiq.com/backend/api/rzp/verify', {
              method: 'POST',
              headers: { 'Content-Type':'application/json' },
              body: JSON.stringify({
                callId: CALL_ID, doctorId: DOCTOR_ID, patientId: PATIENT_ID, channel: CHANNEL,
            ...
            setTimeout(()=>{
              window.location.href = `/call-page/${encodeURIComponent(CHANNEL)}?uid=${encodeURIComponent(PATIENT_ID)}&role=audience&callId=${encodeURIComponent(CALL_ID)}`;
            }, 1200);
          } catch (e) {
        ...
        modal: {
          ondismiss: () => {
            ...
            socket.emit('payment-cancelled', { callId: CALL_ID, patientId: PATIENT_ID, doctorId: DOCTOR_ID, reason: 'user-cancelled' });
          }
        }
      };
  ...
  // ===== Boot =====
  (async () => {
    renderLoadingGateway();
    await loadRazorpay();
    if (!razorpayLoaded) {
      app.innerHTML = `<div class="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full text-center">
