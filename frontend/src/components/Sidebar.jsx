...
const Sidebar = () => {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get(`https://snoutiq.com/backend/chat/history/${user.id}/`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleCreateNewChat = async () => {
    try {
      ...
      const res = await axios.post(
        "https://snoutiq.com/backend/chat/create/",
        { user_id: user.id },
        { headers: { Authorization: `Bearer ${token}` } }
      );

      if (res.data?.chat_room_token) {
        const newChat = { chat_room_token: res.data.chat_room_token, last_message: "" };
  ...
  const handleDeleteChat = async (tokenToDelete) => {
    try {
      ...
      await axios.delete(`https://snoutiq.com/backend/chat/delete/${tokenToDelete}/`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      if (chat_room_token === tokenToDelete) navigate("/dashboard");
    } catch (err) {
      console.error(err);
      toast.error("Failed to delete chat");
    }
  ...
  useEffect(() => {
    ...
    setActivePet(pets[Math.floor(Math.random() * pets.length)]);
  }, []);
        ...
        {isLoading ? (
          ...
          history.map((chat) => (
            <div
              ...
              className={`flex items-center justify-between p-2 rounded-lg cursor-pointer ${chat_room_token === chat.chat_room_token ? "bg-blue-200" : "bg-white"
                }`}
              onClick={() => navigate(`/chat/${chat.chat_room_token}`)}
            >
              <span className="truncate text-sm">{chat.last_message || "New Chat"}</span>
              <button
                onClick={(e) => {
                  e.stopPropagation();
