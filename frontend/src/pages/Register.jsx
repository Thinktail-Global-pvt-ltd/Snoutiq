...
const Register = () => {
  ...
  useEffect(() => {
    checkLocationPermission().then((status) => {
      if (status === true) {
        setLocationAllowed(true);
        requestLocation(); // direct coords bhi le lo
      } else if (status === false) {
        setLocationAllowed(false);
        toast.error("Please enable location access in browser settings");
      } else {
        // null => abhi tak prompt nahi aaya, requestLocation call karne par popup aayega
      }
    });
  }, []);
  ...
  const validateStep = (step = activeStep) => {
    ...
    if (step === 0) {
      ...
      if (!/^\d{10}$/.test(formData.mobileNumber)) newErrors.mobileNumber = 'Valid 10-digit mobile number required';
    }

    if (step === 1) {
      if (!formData.petName.trim()) newErrors.petName = "Pet name is required";
      if (!formData.petGender) newErrors.petGender = "Pet gender is required";
      if (!formData.petAge || formData.petAge <= 0) newErrors.petAge = "Valid pet age is required";
      if (!formData.petBread) newErrors.petBread = "Valid pet Bread is required";
    }
    ...
    return new Promise((resolve) => {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          ...
          // resize karna (agar image badi hai toh)
          let width = img.width;
  ...
  const handleFileChange = async (field, files) => {
    if (files && files.length > 0) {
      ...
      if (!file.type.startsWith("image/")) {
        // agar non-image hai (pdf/doc), toh direct set karo
        handleInputChange(field, file);
        return;
      }

      const compressedFile = await compressImage(file, 50); // 50KB tak compress
      ...
      <div className="min-h-screen bg-white bg-gradient-to-br from-blue-50 to-indigo-100 mt-12 flex items-center justify-center px-4 py-8">
        <div className="w-full max-w-sm sm:max-w-md">
          <Card className="text-center shadow-xl rounded-xl p-6 sm:p-8">
            {/* Logo */}
            <div className="mb-6">
              <img src={logo} alt="Snoutiq Logo" className="h-12 sm:h-14 mx-auto mb-3" />
            </div>

            {/* Welcome Message */}
            <div className="mb-4 sm:mb-6">
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-2">Welcome to Snoutiq!</h1>
              ...
              <div className="flex justify-between items-start relative">
                ...
                {steps.map((label, index) => (
                  <div key={label} className="flex flex-col items-center z-10">
                    ...
                    ${activeStep >= index ? 'bg-blue-600 text-white' : 'bg-white border-2 border-gray-300 text-gray-500'}`}>
                      {activeStep > index ? (
                    ...
                    <p className={`text-xs mt-2 font-medium max-w-20 ${activeStep >= index ? 'text-blue-600' : 'text-gray-500'}`}>
                      {label}
                    </p>
                  </div>
                ))}
                  ...
                  <div className="text-left">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`w-full pl-10 pr-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent ${errors.fullName && touched.fullName ? 'border-red-500' : 'border-gray-300'
                          }`}
                  ...
                  <div className="text-left">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`w-full pl-10 pr-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent ${errors.email && touched.email ? 'border-red-500' : 'border-gray-300'
                          }`}
                  ...
                  <div className="text-left">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`w-full pl-10 pr-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent ${errors.mobileNumber && touched.mobileNumber ? 'border-red-500' : 'border-gray-300'
                          }`}
                ...
                <>
                  <div className="text-left">
                    ...
                    <input
                      ...
                      className={`w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent ${errors.petName && touched.petName ? 'border-red-500' : 'border-gray-300'
                        }`}
                  ...
                  <div className="text-left">
                    ...
                    <select
                      ...
                      className={`w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent ${errors.petGender && touched.petGender ? 'border-red-500' : 'border-gray-300'
                        }`}
                    >
                  ...
                  <div className="text-left">
                    <label className="block text-sm font-medium text-gray-700 mb-2">Pet Bread *</label>
                    <input
                      value={formData.petBread}
                      onChange={(e) => handleInputChange('petBread', e.target.value)}
                      onBlur={() => handleBlur('petBread')}
                      className={`w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent ${errors.petAge && touched.petAge ? 'border-red-500' : 'border-gray-300'
                        }`}
                      placeholder="Enter your pet's Bread"
                    />
                    {errors.petBread && touched.petBread && (
                      <p className="text-red-500 text-xs mt-1">{errors.petBread}</p>
                    )}
                  </div>
                  <div className="text-left">
                  ...
                  <div className="text-left">
                    <label className="block text-sm font-medium text-gray-700 mb-2">Prescription Document (Optional)</label>
                    <div className="flex items-center justify-center w-full">
                      <label className={`flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-lg cursor-pointer ${formData.petDoc1 ? 'border-blue-500 bg-blue-50' : 'border-gray-300 hover:border-gray-400'
                        }`}>
                        <div className="flex flex-col items-center justify-center pt-5 pb-6">
                          ...
                          <p className="mb-2 text-sm text-gray-500">
                            {formData.petDoc1 ? formData.petDoc1.name : <span>Click to upload or drag and drop</span>}
                          </p>
                        </div>
                        <input
                          type="file"
                  ...
                  <div className="text-left">
                    <label className="block text-sm font-medium text-gray-700 mb-2">Medical Document (Optional)</label>
                    <div className="flex items-center justify-center w-full">
                      <label className={`flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-lg cursor-pointer ${formData.petDoc2 ? 'border-blue-500 bg-blue-50' : 'border-gray-300 hover:border-gray-400'
                        }`}>
                        <div className="flex flex-col items-center justify-center pt-5 pb-6">
                          <svg className="w-8 h-8 mb-4 text-gray-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 16">
                            <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 13h3a3 3 0 0 0 0-6h-.025A5.56 5.56 0 0 0 16 6.5 5.5 5.5 0 0 0 5.207 5.021C5.137 5.017 5.071 5 5 5a4 4 0 0 0 0 8h2.167M10 15V6m0 0L8 8m2-2 2 2" />
                          ...
                          <p className="mb-2 text-sm text-gray-500">
                            {formData.petDoc2 ? formData.petDoc2.name : <span>Click to upload or drag and drop</span>}
                          </p>
                        </div>
                        <input
                          type="file"
                  ...
                  <div className="text-left">
                    ...
                    <div className="relative">
                      <input
                        ...
                        onChange={(e) => handleInputChange('emailOtp', e.target.value.replace(/\D/g, ''))}
                      ...
                      {otpStatus === "invalid" && (
                        <p className="text-red-600 text-sm mt-1">❌ Invalid OTP</p>
                      )}
                      <button
                        onClick={handleSendEmailOtp}
                        disabled={isLoading.email || emailOtpCooldown > 0}
                        className={`absolute right-2 top-1/2 transform -translate-y-1/2 text-xs py-2 px-3 rounded ${isLoading.email || emailOtpCooldown > 0
                          ? 'bg-gray-200 text-gray-500 cursor-not-allowed'
                          : 'bg-blue-100 text-blue-700 hover:bg-blue-200'
                          }`}
                      >
                        {isLoading.email ? 'Sending...' : emailOtpCooldown > 0 ? `Resend (${emailOtpCooldown}s)` : 'Send OTP'}
                      </button>
                  ...
                  <div className="text-left">
                    ...
                    <div className="relative">
                      <input
                        ...
                        className={`w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent ${errors.password && touched.password ? 'border-red-500' : 'border-gray-300'
                          }`}
                  ...
                  <div className="text-left">
                    ...
                    <div className="relative">
                      <input
                        ...
                        className={`w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent ${errors.confirmPassword && touched.confirmPassword ? 'border-red-500' : 'border-gray-300'
                          }`}
